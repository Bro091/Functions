...
// Функция, ничего не принимающая и не возвращающая. Выводит текст в консоль
void /*пустота, пусто, пустой*/ say_hello() {
...
// return можно использовать в VOIDовских функциях как break; для остановки
...
int arrays_max(int arr1[], const int length1, int arr2[], const int length2) {
	...
	for (int i = 1; i < length1; i++)
		max = arr1[1] > max ? arr1[i] : max; // элемент массива с индексом  i Больше max, если да, то он максимум иначе max - максимум
...
int main() {
	...
	/*n = 10;
	// std::cout << x2(n) << std::endl; // выводиться на консоль 20
	v_x2(n);
	...
	// Задача 1. Индексы положительных эелемнтов
	/*std::cout << "Задача 1.\nМассив:\n";
	const int size1 = 10;
	int arr1[size1];
	srand(time(NULL));
